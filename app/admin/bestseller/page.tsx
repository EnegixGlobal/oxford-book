...
const updateBestseller = async (id: string, bestseller: boolean) => {
  const token = typeof window !== 'undefined' ? localStorage.getItem('bookhaven-token') : null;
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-4xl font-bold text-gray-900 flex items-center gap-2">
            <Flame className="w-8 h-8 text-red-500" /> Bestseller
          </h1>
          <p className="text-gray-500">Toggle books to appear in the Bestsellers list.</p>
        </div>
        <div className="text-sm text-gray-600">Selected: <span className="font-semibold">{selectedCount}</span></div>
      </div>
                  ...
                  <div className="flex items-center gap-2">
                    <Switch
                      ...
                      onCheckedChange={async (val) => {
                        const prev = b.bestseller;
                        setBooks((arr) => arr.map(x => x._id === b._id ? { ...x, bestseller: !!val } : x));
                        const res = await updateBestseller(b._id, !!val);
                        if (!res?.success) {
                          setBooks((arr) => arr.map(x => x._id === b._id ? { ...x, bestseller: prev } : x));
                          toast.error(res?.message || 'Update failed');
                        } else {
                          toast.success(!!val ? 'Marked as bestseller' : 'Removed from bestsellers');
                        }
                      }}
                    />
                  </div>
