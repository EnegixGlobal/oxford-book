...
import { Plus, Trash2, Edit, GripVertical, Search, X } from 'lucide-react';
...
export default function BookListsPage() {
  ...
  const fetchLists = async () => {
    try {
      const res = await fetch('/api/admin/booklists');
      const data = await res.json();
      if (data.success) {
        setLists(data.data);
      }
    } catch (error) {
      toast.error('Failed to fetch book lists');
    } finally {
      setLoading(false);
    }
  };
  ...
  const openEditDialog = (list: BookList) => {
    setEditingList(list);
    setTitle(list.title);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/booklists?id=${id}`, { method: 'DELETE' });
      const data = await res.json();
      if (data.success) {
  ...
  return (
    <div className="p-8">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">Dynamic Book Lists</h1>
      ...
      <div className="grid gap-4">
        ...
        {lists.length === 0 && (
          <div className="text-center py-12 text-gray-500">
            No book lists yet. Create one to get started.
          </div>
        )}
      </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editingList ? 'Edit Book List' : 'Create Book List'}</DialogTitle>
          </DialogHeader>
          
          <div className="space-y-4 mt-4">
            <div>
              <label className="text-sm font-medium">Title *</label>
              <Input value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g., Staff Picks" />
            </div>
