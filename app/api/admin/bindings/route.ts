...
async function generateUniqueSlug(base: string, excludeId?: string | null) {
	...
	while (true) {
		...
		const exists = await Binding.exists(query);
...
export async function GET(request: NextRequest) {
	...
	try {
		...
		// If there are no bindings yet, seed them first
		...
		const items = await Binding.find(includeInactive ? {} : { isActive: true })
			.sort({ sortOrder: 1, name: 1 })
			.lean();

		return NextResponse.json({ success: true, data: items });
	} catch (error) {
		console.error('Admin bindings GET error:', error);
...
export async function POST(request: NextRequest) {
	...
	try {
		...
		console.error('Admin bindings POST error:', error);
		if (error.code === 11000) {
			return NextResponse.json({ success: false, message: 'Binding with this name already exists' }, { status: 409 });
		}
		return NextResponse.json({ success: false, message: 'Internal server error' }, { status: 500 });
	}
}
...
export async function PUT(request: NextRequest) {
	...
	if (!id) return NextResponse.json({ success: false, message: 'Binding ID is required' }, { status: 400 });
	...
	try {
		...
		await connectDB();
		const updated = await Binding.findByIdAndUpdate(id, update, { new: true });
		if (!updated) {
			return NextResponse.json({ success: false, message: 'Binding not found' }, { status: 404 });
		}

		return NextResponse.json({ success: true, data: updated });
	} catch (error: any) {
		console.error('Admin bindings PUT error:', error);
		if (error.code === 11000) {
			return NextResponse.json({ success: false, message: 'Binding with this name already exists' }, { status: 409 });
		}
		return NextResponse.json({ success: false, message: 'Internal server error' }, { status: 500 });
...
export async function DELETE(request: NextRequest) {
	...
	try {
		await connectDB();
		const deleted = await Binding.findByIdAndDelete(id);
		if (!deleted) {
			return NextResponse.json({ success: false, message: 'Binding not found' }, { status: 404 });
		}
		return NextResponse.json({ success: true, message: 'Binding deleted' });
	} catch (error) {
		console.error('Admin bindings DELETE error:', error);
		return NextResponse.json({ success: false, message: 'Internal server error' }, { status: 500 });
	}
}
