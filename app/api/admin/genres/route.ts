...
async function generateUniqueSlug(base: string, excludeId?: string | null) {
	...
	while (true) {
		...
		const exists = await Genre.exists(query);
...
export async function POST(request: NextRequest) {
	...
	try {
		...
		if (error.code === 11000) {
			return NextResponse.json({ success: false, message: 'Genre with this name already exists' }, { status: 409 });
		}
		return NextResponse.json({ success: false, message: 'Internal server error' }, { status: 500 });
	}
}
...
export async function PUT(request: NextRequest) {
	...
	if (!id) return NextResponse.json({ success: false, message: 'Genre ID is required' }, { status: 400 });
	...
	try {
		...
		await connectDB();
		const updated = await Genre.findByIdAndUpdate(id, update, { new: true });
		if (!updated) {
			return NextResponse.json({ success: false, message: 'Genre not found' }, { status: 404 });
		}

		return NextResponse.json({ success: true, data: updated });
	} catch (error: any) {
		console.error('Admin genres PUT error:', error);
		if (error.code === 11000) {
			return NextResponse.json({ success: false, message: 'Genre with this name already exists' }, { status: 409 });
		}
		return NextResponse.json({ success: false, message: 'Internal server error' }, { status: 500 });
