...
export default function AnticipatedPage() {
  ...
  const load = async (pageToLoad = 1, append = false) => {
    try {
      ...
      const res = await fetch(`/api/books?anticipated=true&page=${pageToLoad}&limit=${limit}`, { cache: 'no-store' });
  ...
  return (
    ...
    >
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-14 relative">
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <h1 className="text-3xl md:text-5xl font-extrabold text-gray-900 tracking-tight">Discover What Everyone Can’t Wait to Read</h1>
            <p className="mt-3 text-lg md:text-xl text-gray-700 max-w-2xl">Handpicked upcoming favorites and buzzing releases—explore, sort, and add to your cart.</p>
          </motion.div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12">
        ...
        >
          <div className="text-gray-700"><span className="font-semibold text-gray-900">{totalItems}</span> anticipated books</div>
            ...
            >
              <div className="relative">
                ...
                <div className="absolute z-10 top-2 left-0">
                  <span className="inline-flex items-center gap-1 rounded-r-full bg-fuchsia-600 text-white text-[11px] font-semibold px-2 py-0.5 shadow">
                    <Sparkles className="w-3 h-3" /> Anticipated
                  </span>
                </div>
                <BookCard book={book} />
              </div>
        ...
        {!loading && sortedBooks.length === 0 && (
          ...
          >
            <h3 className="text-xl font-semibold text-gray-700 mb-2">No anticipated books found</h3>
            <p className="text-gray-500">Check back soon for exciting new releases!</p>
          </motion.div>
        )}
