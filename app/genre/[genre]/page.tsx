...
export default function GenrePage({ params }: { params: Promise<{ genre: string }> }) {
  ...
  const load = async (pageToLoad = 1, append = false) => {
    try {
      ...
      const res = await fetch(`/api/books?genre=${encodeURIComponent(genre)}&page=${pageToLoad}&limit=${limit}`, { cache: 'no-store' });
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-14 relative">
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <h1 className="text-3xl md:text-5xl font-extrabold text-gray-900 tracking-tight">Explore the best in {displayName}</h1>
            <p className="mt-3 text-lg md:text-xl text-gray-700 max-w-2xl">Curated picks and top-rated books in this genre.</p>
          </motion.div>
        </div>
      </div>
