...
export default function AgeGroupPage({ params }: { params: Promise<{ ageGroup: string }> }) {
  ...
  const load = async (pageToLoad = 1, append = false) => {
    try {
      ...
      const res = await fetch(`/api/books?ageGroup=${encodeURIComponent(ageGroup)}&page=${pageToLoad}&limit=${limit}`, { cache: 'no-store' });
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }} className="min-h-screen bg-gradient-to-br from-purple-50 to-pink-50">
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-14 relative">
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <h1 className="text-3xl md:text-5xl font-extrabold text-gray-900 tracking-tight">Perfect picks for {displayName}</h1>
            <p className="mt-3 text-lg md:text-xl text-gray-700 max-w-2xl">Handpicked books tailored for this age group.</p>
          </motion.div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12">
        ...
        >
          <div className="text-gray-700"><span className="font-semibold text-gray-900">{totalItems}</span> books</div>
              ...
              <div className="relative">
                <div className="absolute z-10 top-2 left-0">
                  <span className="inline-flex items-center gap-1 rounded-r-full bg-fuchsia-600 text-white text-[11px] font-semibold px-2 py-0.5 shadow">
                    <Sparkles className="w-3 h-3" /> {displayName}
                  </span>
                </div>
                <BookCard book={book} />
              </div>
